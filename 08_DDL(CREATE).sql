...
-- DDL (DATA DEFINTTION LANGUAGE) : 데이터 정의 언어
...
INSERT INTO MEMBER2 VALUES('MEMO1', 'PASS01', '고길동', 7712341234);
INSERT INTO MEMBER2 VALUES('MEMO2', 'PASS02', '고길순', 01045678901);
--> NUBMER 타입 컬럼에 데이터 삽입 시
-- 제일 앞에 0이 있으면 이를 자동으로 제거함
	 --> 전화번호, 주민등록번호 처럼 숫자로만 되어있는 데이터지만
	 --> 0으로 시작할 가능성이 있으면 CHAR,  VAHCHAR2 같은 문자형을 사용
...
DESC USER_CONSTRAINTS; --SQLPLUS에서만 됨
SELECT * FROM USER_CONSTRAINTS;


-- USER_CONS_COLUMNS : 제약조건이 걸려있는 컬럼을 확인하는 딕서녀리 뷰.
...
-- NULL을 ("KH_OHJ"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- * 제약조건 지정 방법
-- 1) 컬럼 레벨 : [CONSTARAINT 제약조건명] 제약조건
-- 2) 테이블 레벨 : [CONSTARAINT 제약조건명] 제약조건(컬럼명)

-- UINQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK(
	 USER_NO NUMBER,
	 --USER_ID VWARCHAR2(20) UNIQUE, -- 컬럼 레벨 (제약조건명 미지정)
	 --USER_PWD VARCHAR(30) CONSTRAINT USER_PWD_U UNIQUE, -- 컬럼레벨(제약조건명 지정) 
	 USER_ID VARCHAR2(20),
	 USER_PWD VARCHAR(30),
	 USER_NAME VARCHAR2(30),
	 GENDER VARCHAR2(10),
...
INSERT INTO USER_USED_UK
VALUES(1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_OHJ_USER_ID_U)에 위배됩니다
...
INSERT INTO USER_USED_UK2
VALUES(3, 'USER02', 'PASS01', '고길동', '남', '010-1234-5678', 'hong123@kh.or.kr');


INSERT INTO USER_USED_UK2
VALUES(4, 'USER01', 'PASS01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');

-- ORA-00001 : 무결성 제약 조건(KH.OHJ.USER_ID_NAME_U)에 위배됩니다
...
-- NOT NULL + UNIQUE 제약조건의 의미 -> 중복되지 않는 값 필수로 존재해야함.
...
-- 기본키 중복 오류
-- ORA-00001: 무결성 제약 조건(KH_OHJ.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'USER03', 'PASS03', '유관순', '여', '010-9999-5678', 'lee123@kh.or.kr');
-- 기본키 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_OHJ"."USER_USER_PK"."USER_NO")안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_PK2
VALUES(2,'USER01', 'PASS01', '유관순', '여', '010-1234-5678', 'hong123@kh.or.kr');


INSERT INTO USER_USED_PK2
VALUES(1, 'USER01', 'PASS02', '신사임당', '여', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-0001: 무결성 제약 조건(KH_OHJ.PK_USERNO_USERID)에 위배됩니다
-- 회원번호와 아이디 둘다 중복되었을때만 제약조건 위배 에러 발생!
...
-- 테이블레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY (적용할컬럼명) REFERENCES 참조할테이블명 [삭제롤]
...
CREATE TABLE USER_GRADE(
	  GREAD_CODE NUMBER PRIMARY KEY, -- 등급 번호
...
CREATE TABLE USER_USED_FK(
   ...
   USER_ID VARCHAR2(20) UNIQUE,
	 ...
	 PHONE VARCHAR2(30),
	 EMAIL VARCHAR2(50),
	 GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE /*(GREADE_CODE)*/
	 																	-- 컬럼명 미작성 시 USER_GRADE 테이블의 PK를 자동 참조
	-- 테이블 레벨
	--, CONSTRAINT GREADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE
...
-- ORA-02291 : 무결성 제약조건(KH_OHJ.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
DELETE FROM USER_GRADE WHERE GRADE_CODE = 30; -- 30은 자식테이블에서 사용되고 있는 값
 -- ORA_02292: 무결성 제약조건(KH.OHJ.GRADE.CODE_FK)이 위배되었습니다- 자식 레코드
...
-- 2) ON DELETE SET NULL 삭제옵션이 적용된 테이블 생성
CREATE TABLE USER_USED_FK2(
   ...
   USER_ID VARCHAR2(20) ,
	 ...
	 PHONE VARCHAR2(30),
	 EMAIL VARCHAR2(50),
	 GRADE_CDE NUMBER CONSTRAINT GRADE_CODE_FK2 REFERENCES USER_GRADE2 ON DELETE SET NULL 																																/* 삭제 옵션 */
);
...
-- ORA-02290: 체크 제약조건(KH.OHJ.GENDER_CHECK)이 위배되었습니다
...
INSERT INTO USER_TEST
VALUES(2, 'user02', 'pass03', '890222-1234567', '여', '010-2222-9999', '서울시 강남구 삼성동', 'Y');

INSERT INTO USER_TEST
VALUES(3, 'user03', 'pass03', '900122-1234567', '여', '010-3333-9999', '서울시 강남구 종로동', 'Y');

INSERT INTO USER_TEST
VALUES(4, 'user04', 'pass04', '910422-1234567', '남', '010-4444-9999', '서울시 강남구 청계동', 'N');

INSERT INTO USER_TEST
VALUES(5, 'user05', 'pass05', '920522-2234567', '여', '010-5555-9999', '서울시 강남구 가락동', 'Y');
...
-- ALTER TABLE 테이블명 ADD [CONTRAINT 제약조건명] UNIQUE(컬럼명)
-- ALTER TABLE 테이블명 ADD [CONTRAINT 제약조건명] CHECK(컬럼명 비교연산자 비교값)
-- ALTER TABLE 테이블명 ADD [CONTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES 참조테이블명(참조컬럼명)
--> 참조 테이블의 PK(기본키)를 FK로 사용하는 경우 참조컬럼명 생략 가능

-- ALTER TABLE 테이블명 MODIFY 컬럼명 NOT NULL;

-- NOT NULL 제약조건만 복사된 EMPLOYE_COPY 테이블에
