...
-- GRANT RESOURCE, CONNECT TO kh_ohj;

-- 롤 (role)
-- 다수 사용자와 다양한 권한을 효율적으로 관리하기 위햐여
...
CREATE USER ohj_sample IDENTIFIED BY sample1234;


-- 2. 새 연결 추가
--> ORA-01045: 사용자 OHJ_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
...
GRANT CREATE SESSION TO ohj_sample;
...
GRANT CREATE TABLE TO ohj_sample;


ALTER USER ohj_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO ohj_sample;
...
-- kh_ohj /   ohj_sample 사용자 계정끼리 서로 객체 접근 권한 부여


-- 1. (sample) kh_ohj 계정의 EMPLOYEE 테이블 조회
...
GRANT SELECT ON EMPLOYEE TO ohj_sample;
...
REVOKE SELECT ON EMPLOYEE FROM ohj_sample;
