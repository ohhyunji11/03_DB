...
-- 2024-02-29 11:21:55.455 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)
...
SELECT TO_CHAR(1234) FROM DUAL; -- 1,2234 -> '1234'
...
SELECT SYSDATE FROM DUAL; -- 2024-02-29 12:21:16.000
...
-- TO_NUMBER(문자데이터, [포맷]) : 문자데이터를 숫자 데이터로 변경
...
-- NVL(컬럼명, 컬럼값이 NULL일 때 바꿀값) : NULL인 컬럼값을 다른값으로 변경
...
SELECT EMP_NAME, NVL2(BONUS, 'O', 'X') "보너스 수렁"
FROM EMPLOYEE;
...
-- 하나 이상의 그룹으로 묶어 연산하여 총합, 평균 등의 하나의 결과 행으로 반환
...
-- 급여 최대값, 가장느린 입사일, 알파벳순서가 가장느린 이메일을 조회
