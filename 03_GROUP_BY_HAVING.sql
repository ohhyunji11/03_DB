 ...
 * 6 : ORDER BY 컬럼명 || 별칭 || 컬럼순번 정렬방식(ASC/DESC) [NULL FIRS || LAST];
 * 
 * */
...
FROM EMPLOYEE
GROUP BY DEPT_CODE; --> DEPT_CODE가 같은 행끼리 하나의 그룹이됨
...
SELECT DECODE( SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여' ) 성별,
	     ...
	     SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE( SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여' ) -- 별칭사용 X (SELECT 절 해석X)
ORDER BY "인원 수"; -- 별칭사용O (SELECT절 해석 완료)


--------------------------------------------------------------------------


-- * WHERE 절 GROUP BY절을 혼합하여 사용
--> WHERE절은 각 컬럼값에 대한 조건
...
-- EMPLOY 테이블에서 부서코드가 D5, D6인 부서의 평균 급여, 인원 수 조회
...
-- EMPLOYEE 테이블에서 직급 별 2000년도 이후(2000년 포함) 입사사자들의
-- 급여 합을 조회 (직급코드 오름차순)
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
WHERE 
--HIRE_DATE >= T0_DATE('2000-01-01')
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한 사람의 급여가 3백만 이상이라는 조건(요구사항 X)
GROUP BY DEPT_CODE
