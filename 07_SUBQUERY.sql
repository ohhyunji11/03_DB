...
--> 위의 2개의 단계를 하나의 쿼리로 !! --> 1번의 쿼리문을 서브쿼리로
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
/* 서브쿼리 유형
 * 
 * - 단일행 (단일열) 서브쿼리의 조회 결과 값의 개수가 1개일 때
 * 
...
/*
 * >> 다중행 서브쿼링 앞에는 일반 비교연산자 사용 X
 * 
 * - IN / NOT IN : 여러  개의 결과값  중에서 한 개라도 일치하는 값이 있다면
 * 									혹은 없다면 이라는 의미 (가장 많이 사용!)
 * 
 * - > ANY, < ANY : 여러개의 결과 값 중에서 한 개라도 큰 / 작은 경우
 * 									가장 작은 값보다 큰가? / 가장 큰 값 보다 작은가?
 * 
 * - > ALL, < ALL : 여러개의 결과값의 모든 값보다 큰 / 작은 경우
 * 								 가장 큰 값보다 큰가? / 가장 작은 값 보다 작은가?
 * 
 * - EXISTS / NOT EXITST : 값이 존재하는가 ? / 존재하지 않는가?
 * 
 * 
 * */
 ...
 * 실제 데이터 값을 반환하지 않음.
 * 즉, 각 직원의 MANAGER_ID가 다른 직원의 EMP_ID가 일치하는 경우만 확인됨
 * 
...
-- 메인쿼리(사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일,
--                                           단, 퇴사한 직원은 제외)
 ...
 * 3. 전지연 입사일이랑 같은. 전지연이 D1 부서의 가장 빠른 입사자
 * (RESULT SET 포함)
 * 
 * 
 * 
...
-- 서브쿼리
SELECT EMP_NAME 이름, DEPT_TITLE부서
FROM EMPLOYEE
...
-- 1. 전지연 사원이 속해있는 부서명을 조회하시오(단, 전지연은 제외)
-- 사번, 사원명, 전화번호, 고용일, 부서명
